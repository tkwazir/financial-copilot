...
Three features on top of the base pipeline:
- **Price chart(s)** — any question whose generated SQL references tickers (extracted from the SQL itself, not the raw question) renders an interactive chart below the answer for each one, up to 4 — so "compare MSFT vs GOOGL vs AAPL vs AMZN" shows all four side by side, not just one. Each chart: full price history, hover for date/price, period buttons (1W/1M/3M/6M/1Y/ALL — daily-resolution only, since the underlying data has no intraday ticks).
...
- **News-grounded "why" answers** — a lightweight intent classifier routes questions like "why is NVDA moving?" or "why is the market up today?" away from the NL-to-SQL pipeline entirely and into a real-news RAG path: fetch real, publisher-attributed headlines via `yfinance`'s `.news` (no new API key — same free library already used for prices), then have Claude answer *using only those headlines*, citing the publisher for every claim. If nothing relevant comes back, the answer says so plainly rather than inventing a reason — verified live (a market-wide "why" question with no macro news available correctly returned "no relevant recent news was found" instead of fabricating one). Cited sources render as clickable links in the UI, same transparency principle as showing the SQL.

...
- `app/main.py` — FastAPI app. `GET /health`, `POST /query` (`{"question": "..."}` -> SQL, accept/reject, answer, row count, timing). Schema description is introspected once at startup (`lifespan`) and cached.
...
- Next.js chat frontend (Phase 8)
- 20-30 question SQL accuracy evaluation, reported as a real percentage (Phase 9)

...
- `app/main.py` — FastAPI app. `GET /health`, `POST /query` (`{"question": "..."}` -> SQL, accept/reject, answer, row count, timing). Schema description is introspected once at startup (`lifespan`) and cached. Also `GET /chart/{ticker}` (full daily-close history for one ticker) and `GET /tickers` (latest close + day change + a 20-day sparkline for every ticker) — both fixed, parameterized, non-LLM-generated queries powering the frontend's price chart and ticker tape, so no guardrail layer applies to them.

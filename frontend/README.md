...
- `app/page.tsx` — the ledger UI (client component): ticker tape, input, example-question chips (shown only before the first question), entry list. The input shows a ghost-text suggestion ("Compare MSFT vs GOOGL") on first load only — accept it with the right-arrow key, or a double-click into the field (a single click just focuses normally, so a visitor can still start typing their own question without triggering it), or just type over it; once dismissed (typed into, accepted, or a chip clicked) it never reappears, even if the field is cleared again. Entries render **newest first**, immediately below the input — the input lives at the top of the page (not a sticky bottom bar), so appending newest-last would bury each new answer at the bottom of a growing list; question numbering (`01`, `02`, ...) still reflects the order asked, not display position.
